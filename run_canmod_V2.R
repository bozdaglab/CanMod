...
load("variable.selection.data.rda")
...
#regression.data contains mRNA, cna, methyl, miRNA data and interractions data as null
{
  samples = colnames(regression.data$mRNA)[grep(colnames(regression.data$mRNA), pattern = "Tumor")]
  #samples contain all samples who have tumor in regression.data$mRNA data
  ...
  #line 47 contains transcription factors that are common between TCGA and TRED and TTRUST
  ...
  #line 49 contains transcription factor's target that are common between TCGA and TRED and TTRUST
...
#class(mRNA.targets)
cluster = parallel::makeCluster(parallel::detectCores()-5, outfile = "log/nov20.out")
